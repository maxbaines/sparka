...
interface ArtifactContextType {
  ...
  setMetadata: (documentId: string, metadata: any | ((current: any) => any)) => void;
}
  ...
  const setMetadata = useCallback(
    (documentId: string, metadata: any | ((current: any) => any)) => {
      setMetadataStore((current) => ({
        ...current,
        [documentId]: typeof metadata === 'function' 
...
export function useArtifact() {
  const { artifact, setArtifact, metadata: metadataStore, setMetadata: setMetadataStore } = useArtifactContext();

  const metadata = useMemo(() => {
    return artifact.documentId ? metadataStore[artifact.documentId] || null : null;
